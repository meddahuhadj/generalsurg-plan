...
logger = logging.getLogger("orlsurgplan3d.chat")
...
ACTION_COMMAND_INSTRUCTIONS = (
    ...
    "switch_laryngologie, switch_otologie, switch_rhinologie, switch_cervicofacial, "
    "switch_pediatrique, switch_anesthesie_reanimation."
)
...
async def chat(req: ChatRequest, request: Request, current: models.User = Depends(get_current_user)):
    ...
    system_prompt = (
        f"Tu es ORLSurgPlan3D IA, assistant chirurgical expert en {label}. "
...
async def ws_chat_stream(ws: WebSocket):
    ...
    specialty = data.get("specialty", "laryngologie")
